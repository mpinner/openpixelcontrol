...
// TLC5947 datasheet lists a max rate of 25mhz, anecdotal evidence 
// from forum posts suggests that most people have had trouble 
// obtaining that in practice.
#define TLC5947_DEFAULT_SPEED 4000000

// Different TLC5947 strips expect different input color orderings.
typedef enum { RGB=0, GRB=1, BGR=2 } order_t;
#define DEFAULT_INPUT_ORDER RGB

static int spi_fd = -1;
static u8 spi_data_tx[1 << 16];
static u32 spi_speed_hz = TLC5947_DEFAULT_SPEED;
static order_t rgb_order = DEFAULT_INPUT_ORDER;

void tlC5947_put_pixels(int fd, u8 spi_data_tx[], u32 spi_speed_hz, 
...
void handler(u8 address, u16 count, pixel* pixels) {
  ...
  fflush(stderr);
  tlC5947_put_pixels(spi_fd, spi_data_tx, spi_speed_hz, count, 
    pixels, rgb_order);
}
